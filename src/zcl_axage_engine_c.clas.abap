CLASS zcl_axage_engine_c DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    METHODS interprete
      IMPORTING
        command               TYPE clike
      CHANGING
        ch_o_abapaxage_custom TYPE REF TO zcl_abapaxage_custom OPTIONAL
      RETURNING
    ...
    METHODS get_inventory
      RETURNING
        VALUE(result) TYPE REF TO zcl_axage_result_c.
    DATA player TYPE REF TO zcl_axage_actor_c.
    DATA map TYPE REF TO zcl_axage_map_c.
    DATA actors TYPE REF TO zcl_axage_thing_list_c.
    DATA mission_completed TYPE abap_bool.
  PROTECTED SECTION.
  PRIVATE SECTION.
    METHODS cmd_look
      IMPORTING
        result TYPE REF TO zcl_axage_result_c
        cmd2   TYPE string OPTIONAL.
ENDCLASS.



CLASS zcl_axage_engine_c IMPLEMENTATION.


  METHOD cmd_look.

    IF cmd2 IS INITIAL.
      DATA actor TYPE REF TO zcl_axage_actor_c.
      ...
      IF player->location->east->name <> zcl_axage_room_c=>no_exit->name.
        result->add( 'There is a door on the east side' ).
      ENDIF.
      IF player->location->west->name <> zcl_axage_room_c=>no_exit->name.
        result->add( 'There is a door on the west side' ).
      ENDIF.
      IF player->location->north->name <> zcl_axage_room_c=>no_exit->name.
        result->add( 'There is a door on the north side' ).
      ENDIF.
      IF player->location->south->name <> zcl_axage_room_c=>no_exit->name.
    ...
    CASE cmd1.
      ...
      WHEN 'OPEN'.
        ...
        ELSE.
          ...
          IF thing IS INSTANCE OF zcl_axage_openable_thing_c.
            DATA(thing_to_open) = CAST zcl_axage_openable_thing_c( thing ).
      ...
      WHEN 'ASK'.
        DATA actors_in_the_room TYPE STANDARD TABLE OF REF TO zcl_axage_actor_c.
        DATA actor TYPE REF TO zcl_axage_actor_c.
