...
CLASS ltcl_engine DEFINITION FINAL FOR TESTING
  ...
  PRIVATE SECTION.
    DATA cut TYPE REF TO zcl_axage_engine_c.
    METHODS:
      setup,
...
CLASS ltcl_engine IMPLEMENTATION.
  ...
  METHOD map_simple.
    ...
    DATA(room_left) = NEW zcl_axage_room_c( name = 'LEFT' descr = 'Left room' ).
    DATA(room_right) = NEW zcl_axage_room_c( name = 'RIGHT' descr = 'Right room' ).
  ...
  METHOD take_and_drop.
    ...
    DATA(room_right) = NEW zcl_axage_room_c( name = 'RIGHT' descr = 'Right room' ).
    room_left->set_exits( e = room_right ).
    DATA(bottle) = NEW zcl_axage_thing_c( name = 'BOTTLE' descr = 'an empty bottle' ).
    ...
    cl_abap_unit_assert=>assert_equals(
      ...
      exp = VALUE zcl_axage_thing_list_c=>_things( (  bottle ) ) ).
